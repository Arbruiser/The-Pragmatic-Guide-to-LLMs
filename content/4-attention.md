...
- **Absolute positional encoding:** Because attention looks at all tokens simultaneously, word order would otherwise be invisible — "the dog bit the man" would look identical to "the man bit the dog". In the classic 2017 transformer design, a unique "position vector" was calculated using a fixed mathematical formula for every single position from the beginning of the input (one vector for position 1, a different vector for position 2, etc.) and added directly to the token's embedding. Later models tried learning these absolute position vectors during training, but this meant they couldn't generalise to text lengths longer than they saw in training, because the model had never learned what the unique vector for position 10,001 should look like.

> [!note] Relative distance and RoPE
> Most modern LLMs use **RoPE** (Rotary Position Embeddings), which completely replaces absolute position vectors. Instead of modifying the embedding at the start, RoPE *rotates* the Query and Key vectors inside every attention layer by an angle proportional to their position. Thanks to the mathematics of rotation, when the model calculates the match (dot product) between a rotated Query and a rotated Key, the absolute positions mathematically cancel out, and the result depends purely on the *relative distance* between the two tokens. This is intuitively similar to how humans read: we care about the distance between words ("is this adjective right next to this noun?"), not their absolute position from the start of the text. While vanilla RoPE actually struggles to extrapolate to longer contexts on its own, it provides a mathematical foundation that allows developers to easily apply scaling tricks (like YaRN or NTK-aware interpolation) to stretch the context window to millions of tokens.
...
- **MLA (Multi-Head Latent Attention):** Developed by DeepSeek, this takes a radically different approach. Instead of sharing **K**/**V** heads directly, MLA compresses the key and value tensors into a much smaller, lower-dimensional space (a single "Latent Vector") before storing them in the KV cache. At inference time, these compressed vectors are projected back to their original size. While the attention compute itself remains quadratic (it still has to check all tokens against all tokens), MLA drastically reduces the KV cache size. Though it is slightly larger than an MQA cache, it manages to recover MHA-level quality without the massive memory overhead.
...
The solution is remarkably simple. Instead of a pure sliding window, you permanently pin the first "sink" token in the KV Cache, and use the remaining slots for a rolling window of the most recent tokens. This keeps the attention mechanism mathematically stable: by ensuring the universally visible first token never falls outside the context window, the model avoids crashing into nonsense and can generate coherent text indefinitely.
